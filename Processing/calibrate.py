...
import ocr
import warnings
from classes import Calibration, CFrame, Transformer
...
# ------------------------------------------------------------
# CALIBRATION: Given a calibration trial, calibrate it! Outputs a calibration with a Transformer;
# The transformer is not only cached with the outputted calibration file but also is saved inside 
# of the trial's directory as its own file. This can be referenced as an example of generating 
# transformers for use elsewhere.
# ------------------------------------------------------------
...
def calibrate(  
    ...
    targets_filepath = os.path.join(root_dir, targets_filename)
    ...
    targets_df = pd.read_csv(targets_filepath)                      # Read csv file for calibration targets
    ...
    target_frames = df.set_index('frame').to_dict(orient="index")   # Extract the df as a dictionary, with frames as the dictionary keys
    pbar.update(1)
    ...
    for frame in frames:
        ...
        calibration.transformer.add_vr_coords(frame.vr_coords)
        calibration.transformer.add_img_coords(frame.img_coords)
    pbar.update(1)

    # Calculate transformation matrix
    pbar.set_description(f"Calculating the transformation matrix...")
    calibration.transformer.calculate_transform()
    pbar.update(1)
    ...
    if validate:
        ...
        validation_outdir = h.mkdirs(os.path.join(calibration.root_dir, 'calibrations'))
        validation_errors = []
        for frame in frames:
            vr_coords = frame.vr_coords
            img_coords = frame.img_coords
            estimation = calibration.transformer.screen_to_frame(frame.vr_coords)
